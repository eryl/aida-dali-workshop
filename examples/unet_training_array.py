...
def main(dataset_dir: Path):
    ...
    images = sorted(dataset_dir.glob("im*.nii.gz"))
    segs = sorted(dataset_dir.glob("seg*.nii.gz"))

    transform_list = []
    aug_prob = 0.3
    transform_list.append(RandRotate( prob=aug_prob, range_x = [-1.5, 1.5], range_y = [-1.5, 1.5],range_z = [-1.5, 1.5], mode = "bilinear",padding_mode="zeros")),
    transform_list.append(RandAxisFlip(prob=aug_prob)),
    transform_list.append(RandZoom(min_zoom = 0.9,max_zoom = 1.1, mode = 'bilinear', prob=aug_prob)), 
    transform_list.append(RandSpatialCrop((96, 96, 96), random_size=False))
